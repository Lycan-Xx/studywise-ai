...
/**
 * TestTaking — improved mobile UX and robustness:
...
export function TestTaking({
    ...
    const handleSaveForLater = () => {
        const savedSession = saveTestForLater();
        if (savedSession) {
            setShowSaveForLaterDialog(true);
        }
    };
    ...
    // ---------- RENDER ----------
    return (
        <div className="flex flex-col h-[100dvh] md:h-auto bg-[var(--studywise-bg)]">
            {/* MOBILE LAYOUT */}
                ...
                <div className="flex-shrink-0 bg-white border-b border-gray-200 px-4 py-2">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            {timeDisplay && (
                                <div
                                    className={`flex items-center gap-1 px-3 py-1 rounded-full text-sm font-medium ${isTimeRunningOut ? "bg-red-100 text-red-700" : "bg-gray-100 text-gray-700"
                                        }`}
                                    aria-live="polite"
                                >
                                ...
                                {currentQuestion.options.map((option, index) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`border rounded-xl p-4 cursor-pointer transition-all active:scale-[0.98] flex items-start gap-3 min-h-[56px] ${selected
            ...
            <div className="hidden md:block w-full max-w-4xl mx-auto p-6">
                ...
                {timeDisplay && (
                    <div className="flex justify-center mb-8" aria-live="polite">
                        <div
                            className={`flex items-center gap-6 px-8 py-4 rounded-2xl border-2 shadow-lg ${isTimeRunningOut ? "bg-red-50 border-red-200" : "bg-white border-gray-200"
                                }`}
                        >
                            <div className="text-center">
            ...
            <AlertDialog open={showSubmitDialog} onOpenChange={setShowSubmitDialog}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            You have answered {Object.keys(answers).length} out of {progress.total} questions.
            ...
            <AlertDialog open={showSaveForLaterDialog} onOpenChange={setShowSaveForLaterDialog}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>Test Saved Successfully!</AlertDialogTitle>
                        <AlertDialogDescription>
                            Your test progress has been saved. You can resume this test anytime from your library.
                            You answered {Object.keys(answers).length} out of {progress.total} questions.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogAction
                            onClick={() => {
                                setShowSaveForLaterDialog(false);
                                if (onBack) onBack();
                            }}
