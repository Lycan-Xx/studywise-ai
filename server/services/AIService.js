...
class MultiProviderAIService {
    ...
    getAvailableProvider(options) {
        ...
        const availableProviders = Array.from(this.providers.entries())
            ...
            // If priority is same, prefer Gemini
    ...
    async makeGeminiRequest(model, prompt) {
        const geminiKey = process.env.GEMINI_API_KEY || process.env.VITE_GEMINI_API_KEY;
        if (!geminiKey)
    ...
    async makeOpenAIRequest(model, prompt) {
        const openaiKey = process.env.OPENAI_API_KEY || process.env.VITE_OPENAI_API_KEY;
        if (!openaiKey)
    ...
    async makeAnthropicRequest(model, prompt) {
        const anthropicKey = process.env.ANTHROPIC_API_KEY || process.env.VITE_ANTHROPIC_API_KEY;
        if (!anthropicKey)
    ...
    async makeDeepSeekRequest(model, prompt) {
        const deepseekKey = process.env.DEEPSEEK_API_KEY || process.env.VITE_DEEPSEEK_API_KEY;
        if (!deepseekKey)
    ...
    async makeKimiRequest(model, prompt) {
        const kimiKey = process.env.MOONSHOT_API_KEY || process.env.VITE_MOONSHOT_API_KEY || process.env.KIMI_API_KEY;
        if (!kimiKey)
    ...
    async makeHuggingFaceRequest(model, prompt) {
        const hfToken = process.env.HUGGINGFACE_API_TOKEN || process.env.VITE_HUGGINGFACE_API_TOKEN;
        if (!hfToken) {
    ...
    async makeMockRequest(model, prompt) {
    ...
    async makeProviderRequest(providerId, prompt) {
     ...
     */
    async executeWithFailover(prompt, options) {
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            const availableProviders = Array.from(this.providers.entries())
                .filter(([id, p]) => p.available && p.requestCount < p.maxRequests)
...
Generate ${questionCount} questions now:`;
    ...
    processGeneratedQuestions(parsedResponse, options) {
    ...
    findBestSourceMatch(sourceText, content) {
    ...
    async generateTestInsights(testResult) {
        const wrongQuestions = testResult.questions.filter((q) => testResult.userAnswers[q.id] !== testResult.correctAnswers[q.id]);
...
Provide insights in JSON format:
{
  ...
  "focusAreas": ["2-3 specific topics to review"]
    ...
    generateBasicInsights(testResult) {
    ...
    getCachedQuestions(contentHash) {
    ...
    setCachedQuestions(contentHash, response) {
